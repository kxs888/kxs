...
class Validate
{
    ...
    public function check($data, $rules = [], $scene = '')
    {
        ...
        foreach ($rules as $key => $item) {
            ...
            // 场景检测
            if (! empty($scene)) {
                if ($scene instanceof \Closure && ! call_user_func_array($scene, [
                    $key,
                    $data
                ])) {
                    continue;
        ...
        foreach ($rules as $key => $rule) {
            if ($rule instanceof \Closure) {
                ...
                if (0 === strpos($info, 'require') || (! is_null($value) && '' !== $value)) {
                    // 验证类型
                    $callback = isset(self::$type[$type]) ? self::$type[$type] : [
                        $this,
                        $type
                    ];
                    // 验证数据
    ...
    protected function is($value, $rule, $data = [])
    {
        switch ($rule) {
            ...
            case 'file':
                ...
                break;
            case 'image':
                $result = $value instanceof File && in_array($this->getImageType($value->getRealPath()), [
                    1,
                    2,
                    3,
                    6
            ...
            default:
                if (isset(self::$type[$rule])) {
                    // 注册的验证规则
                    $result = call_user_func_array(self::$type[$rule], [
                        $value
                    ]);
                } else {
                    // 正则验证
    ...
    protected function ip($value, $rule)
    {
        ...
        return $this->filter($value, [
            FILTER_VALIDATE_IP,
            'ipv6' == $rule ? FILTER_FLAG_IPV6 : FILTER_FLAG_IPV4
        ]);
    }
    ...
    protected function image($file, $rule)
    {
        ...
        if ($rule) {
            ...
            return in_array($this->getImageType($file->getRealPath()), [
                1,
                2,
                3,
                6
