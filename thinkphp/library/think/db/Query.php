...
class Query
{
    ...
    public function count($field = '*')
    {
        if (isset($this->options['group'])) {
            ...
            $subSql = $this->options($options)
                ...
                ->buildSql();
            return $this->table([
                $subSql => '_group_count_'
            ])->value('COUNT(*) AS tp_count', 0, true);
        }
    ...
    protected function parseWhereExp($logic, $field, $op, $condition, $param = [])
    {
        ...
        if ($field instanceof \Closure) {
            $this->options['where'][$logic][] = is_string($op) ? [
                $op,
                $field
            ] : $field;
            return;
    ...
    public function whereTime($field, $op, $range = null)
    {
        if (is_null($range)) {
            ...
            switch (strtolower($op)) {
                ...
                case 'last year':
                    $range = [
                        mktime(0, 0, 0, 1, 1, $date['year'] - 1),
                        mktime(0, 0, 0, 1, 1, $date['year'])
                    ];
                    break;
                default:
                    $range = $op;
    ...
    public function withCount($relation, $subQuery = true)
    {
        if (! $subQuery) {
            ...
            foreach ($relations as $key => $relation) {
                ...
                $this->field([
                    $count => Loader::parseName($relation) . '_count'
                ]);
            }
        }
        return $this;
    ...
    protected function parsePkWhere($data, &$options)
    {
        ...
        if (is_string($pk)) {
            ...
            // 根据主键查询
            if (is_array($data)) {
                $where[$key] = isset($data[$pk]) ? $data[$pk] : [
                    'in',
                    $data
                ];
            } else {
                $where[$key] = strpos($data, ',') ? [
                    'IN',
                    $data
                ] : $data;
            }
