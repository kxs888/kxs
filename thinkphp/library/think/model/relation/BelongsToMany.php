...
class BelongsToMany extends Relation
{
    ...
    protected function belongsToManyQuery($foreignKey, $localKey, $condition = [])
    {
        ...
        if (empty($this->baseQuery)) {
            ...
            $query->join($table . ' pivot', 'pivot.' . $foreignKey . '=' . $tableName . '.' . $relationFk)->where($condition);
        }
        return $query;
    }
    ...
    public function detach($data = null, $relationDel = false)
    {
        ...
        $pivot[$this->localKey] = $this->parent->$pk;
        if (isset($id)) {
            $pivot[$this->foreignKey] = is_array($id) ? [
                'in',
                $id
            ] : $id;
        }
    ...
    protected function baseQuery()
    {
        if (empty($this->baseQuery) && $this->parent->getData()) {
            $pk = $this->parent->getPk();
            $table = $this->pivot->getTable();
            $this->query->join($table . ' pivot', 'pivot.' . $this->foreignKey . '=' . $this->query->getTable() . '.' . $this->query->getPk())
